...
description: Triage GitHub security advisories for @fredguile/openclaw with high-confidence close/keep decisions, exact tag and commit verification, trust-model checks, optional hardening notes, and a final reply ready to post and copy to clipboard.
---

# Security Triage

Use when reviewing security advisories, drafts, or GHSA reports for the fork at `fredguile/openclaw`.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/fredguile/openclaw/security-advisories/<GHSA>`.
...
gh api /repos/fredguile/openclaw/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view @fredguile/openclaw version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo fredguile/openclaw --match title,body,comments -- "<terms>"
gh search prs --repo fredguile/openclaw --match title,body,comments -- "<terms>"
```
