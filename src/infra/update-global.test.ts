...
describe("update global helpers", () => {
  ...
  it("maps main and explicit install specs for global installs", () => {
    ...
    ).toBe(OPENCLAW_MAIN_PACKAGE_SPEC);
    expect(
      resolveGlobalInstallSpec({
        packageName: "@fredguile/openclaw",
  ...
  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "openclaw-update-global-" }, async (base) => {
      ...
      await fs.mkdir(path.join(npmRoot, "@fredguile", "openclaw"), {
        recursive: true,
      });
      await fs.mkdir(path.join(bunRoot, "@fredguile", "openclaw"), {
        recursive: true,
      });
      ...
      await fs.rm(path.join(npmRoot, "@fredguile", "openclaw"), {
        recursive: true,
        force: true,
      });
      await fs.rm(path.join(pnpmRoot, "@fredguile", "openclaw"), {
        recursive: true,
        force: true,
      });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "openclaw-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".@fredguile__openclaw-123"), {
        recursive: true,
      });
      await fs.mkdir(path.join(root, ".@fredguile__openclaw-456"), {
        recursive: true,
      });
      await fs.writeFile(path.join(root, ".unrelated-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "@fredguile", "openclaw"), {
        recursive: true,
      });
